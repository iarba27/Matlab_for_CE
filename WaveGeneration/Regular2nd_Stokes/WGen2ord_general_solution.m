...
%Regulas waves and Calculate the time series of a PISTON- and FLAP-type wavemaker for
...
z=-0.1; %z coordinate ADV
g=9.81; %gravity acceleration
ph0=0; %initial phase
l=Inf; %hing lenght (Inf.=piston, 0=flap)
...
m1=(4*sinh(kL*d)/(sinh(2*kL*d)+2*kL*d))*(sinh(kL*d)+(1-cosh(kL*d))/(kL*(d+l))); %1st order Biesel function S/H
s0=H/m1; % max Stroke at Still Water Level 

o2=(H^2)/((32*d)*(1-d/(2*(d+l))))*(3*cosh(kL*d)/(sinh(kL*d)^3)-2/m1); %2nd order component
...
for i=1:row;
        ...
        %% piston displacement 
        e1(i,1)=0.5*H/m1*sin(2*pi*f*time(i)+ph0); %1st term
